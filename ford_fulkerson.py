...
def build_resource_constraint_graph(
        ordered_activities, durations,
        num_resources=1
        ):
    """
...
def build_precedence_graph(ordered_activities, durations):
    ...
    Le max flow représente le nombre maximum d'activités
...
def build_time_expanded_graph(ordered_activities, durations):
    """
    Construit un graphe temporel étendu pour
    modéliser l'utilisation des ressources dans le temps.
    """
    G = nx.DiGraph()
    source = "source"
    sink = "sink"
    ...
    # Connexion entre nœuds de temps
    # consécutifs (capacité = nombre de ressources)
...
def compute_max_flow_multiple_methods(ordered_activities, durations):
    ...
    try:
        G1, source1, sink1 = build_precedence_graph(
            ordered_activities,
            durations
            )
        flow1, _ = nx.maximum_flow(
            G1, source1,
            sink1,
    ...
    try:
        G2, source2, sink2 = build_resource_constraint_graph(
            ordered_activities,
            durations
            )
        flow2, _ = nx.maximum_flow(
            G2, source2,
            sink2,
            flow_func=nx.algorithms.flow.edmonds_karp
            )
    ...
    try:
        # Pour un ordonnancement séquentiel,
        # le max flow est le nombre d'activités
        # qui peuvent être effectivement exécutées
        feasible_activities = len(
            [act for act in ordered_activities if act <= len(durations)]
            )
        results['direct'] = feasible_activities
        max_flow = max(max_flow, feasible_activities)  # Mise à jour d maxflow
    except Exception as e:
...
def analyze_schedule_quality(ordered_activities, durations):
    ...
    total_duration = sum(
        durations[act - 1] for act in ordered_activities if act <= len(
            durations
            )
        )
    ...
    valid_activities = [
        act for act in ordered_activities if 1 <= act <= len(durations)
        ]
    metrics['valid_activities'] = len(valid_activities)
    metrics['invalid_activities'] = (
        len(ordered_activities) - len(valid_activities)
        )

    # Utilisation des ressources (supposant 1 ressource)
    if total_duration > 0:
        metrics['resource_utilization'] = (
            len(valid_activities) / total_duration
            )
    else:
...
def export_results_to_csv(all_instances_results, output_path):
    """
    Exporte les résultats de tous les algorithmes
    dans un fichier CSV pour comparaison.
    """
            ...
            for line in f:
                ...
                if 'duree' in line or 'durations' in line:
                    ...
                    if match:
                        ...
                        durations = [
                            int(x) for x in values.replace(',', ' ').split()
                            ]
                        break
    except Exception as e:
        print(f"Erreur lors du chargement de {filepath}: {e}")
...
def get_instances_from_dzn():
    """
    Retourne la liste des noms d'instances (sans extension)
    présents dans le dossier INSTANCES_DIR.
    """
    instances = []
...
def main():
    ...
    for instance in sorted(common_instances):
        ...
        for algo in ALGOS:
            ...
            max_flows, max_flow = compute_max_flow_multiple_methods(
                ordered,
                instance_durations
                )
    ...
    if all_instances_results:
        ...
        csv_comparison_path = os.path.join(
            FORD_FULKERSON_DIR,
        ...
        # CSV détaillé
        csv_detailed_path = os.path.join(
            FORD_FULKERSON_DIR,
            "resultats_detailles.csv"
            )
